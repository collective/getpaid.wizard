...
class IWizardController(interface.Interface):
    ...
    def getTraversedFormSteps():
        """
        get all the form steps that have been traversed to the current step, to
        revalidate their input into data adapters.

        typically this is the path from start point to current. full stateful
        workflows should not attempt to get this from the request! as its not
        trusted input, and should try storing persistently or in session.
        """
...
class IWizardStep(IContentProvider):
    ...
    next_step_name = schema.ASCIILine(
        ...
        description=u"""After update this is set to the next step to use
        in a wizard, typical values, include interfaces.WIZARD_PREVIOUS_STEP,
        or WIZARD_NEXT_STEP"""
    )
